...
4. **[Gene Names, Gene Ranking, CNVs, Small Variants and Multi-omics Query Output](#!/help#genes-gene-rank-cnvs-vcf-and-multi-omics-query-output)**
  ...
  2. [Scatter Plot](#!/help#scatter-plot)
  3. [Pie Charts](#!/help#pie-charts)
  4. [Summary Table](#!/help#summary-table)
  5. [PanDrugs Report](#!/help#report)
...
6. **[Examples](#!/help#examples)**
  ...
  5. [Small Variants Analysis with Somatic Alterations](#!/help#examples-vcf-query-somatic-variants)
  6. [Small Variants Analysis with Somatic and Germline Alterations](#!/help#examples-vcf-query-somatic-germline-variants)
  ...
  7. [Multi-omics Analysis](#!/help#examples-multi-omics-query)
...
There are five types of queries to obtain a **rank of therapies according to an individualized genomics profile**, depending on the input data:
  ...
  - **[CNVs query](#!/help#cnvs-query):** Using a list of altered genes and their CNV status in .tsv format.
  - **[Small variants query](#!/help#vcf-query):** Using a VCF containing a set of somatic (and optionally germline) variants corresponding to the GRCh38/hg38 assembly of the human genome.
...
<div style="text-align: left;"><img src="genes-query-01.png" alt="Gene Names Query" height="40%" width="40%" max-width="850px"/>
...
<div style="text-align: left;"><img src="drug-query-01.png" alt="Drug Query" height="40%" width="40%" max-width="850px"/>
...
This file must consist of two tab-delimited columns containing gene symbols and the ranking metric. Each gene symbol must appear in a different line. 

The ranking metric will be scaled between 0 and 1 and treated as a user-supplied [GScore](#!/help#gscore-calculation), overwriting the pre-computed one.

<div style="text-align: left;"><img src="gene-ranking-query-01.png" alt="Gene Ranking Query" height="40%" width="40%" max-width="850px"/>
...
<div style="text-align: left;"><img src="cnvs-query-01.png" alt="CNVs Query" height="40%" width="40%" max-width="850px"/>
...
<div style="text-align: left;"><img src="vcf-query-01.png" alt="Small Variants Query" height="40%" width="40%" max-width="850px"/>
...
<div style="text-align: left;"><img src="vcf-query-03.png" alt="Small Variants Query Submission Message" height="20%" width="20%" max-width="850px"/>
...
<div style="text-align: left;"><img src="vcf-query-04.png" alt="Progress Bar" height="40%" width="40%" max-width="850px"/>
...
<div style="text-align: left;"><img src="vcf-query-05.png" alt="Progress Bar Completed" height="40%" width="40%" max-width="850px"/>

If you are logged in PanDrugs, the results of the query will be stored in your account. You will be able to access any previous analyses done within the last 6 months and select one of them to make a new query. You can register [here](https://pandrugs.sing-group.org/#!/login). **Note that the germline variants used to query PharmCAT are deleted immediately and not stored in our servers**.
...
<div style="text-align: left;"><img src="multi-omics-query-01.png" alt="Multi-omics Query" height="40%" width="40%" max-width="850px"/>
...
This file must consist of two tab-delimited columns containing gene symbols and the ranking metric. Each gene symbol must appear in a different line and **the ranking metric must reflect the expression change between tumour and normal samples** (e.g. a differential expression test statistic).
...
Then, PanDrugs will query its database with the genes with known CNV status information, if available. Moreover, if there is a VCF, PanDrugs runs a small variants query (with an optional call to PharmCAT). If a gene has small variants and CNVs, the GScore computed from VScores has priority. The output table will contain annotations for each drug-associated gene, including its SNV, CNV and expression status, when available.
...
<div style="text-align: left;"><img src="analysis-options.png" alt="Analysis Options" height="40%" width="40% max-width="850px"/>

### 2.1 Drug Status<a name="drugs-status"></a>
...
PanDrugsdb stores pre-computed DScores for each drug-gene pair. This score has been calculated according to the drug indication for cancer or other diseases, its approval status and the type of direct drug-gene association (direct target or biomarker). Moreover, the sign of the pre-computed DScore indicates the direction of the drug response (sensitivity or resistance).

<div style="text-align: left;"><img src="precomputed-dscore.png" alt="Pre-computed DScores" height="30%" width="30%" max-width="850px"/>
...
Depending on the type of query, the GScore can suffer modifications:
  ...
  - **After a multi-omics query:** There can be two types of GScores:
    ...
    If a gene has small variants and CNVs, the GScore computed from VScores has priority.
...
Once the query has been completed, a summary box with the execution details, some plots, a summary table with a drug ranking and a downloadable report with top results will appear.

### 4.1 Summary Box<a name="summary-box"></a>

This box details the total number of queried genes as well as the number of genes present and absent in PanDrugsdb. Moreover, it specifies the type of query in the title and the [analysis options](#!/help#analysis-options) that were selected. If you made a [small variants query](#!/help#vcf-query), the summary box will display a button to download the [PanDrugs report](#!/help#report). In addition, if you did a <span style="color:#50AC50">**PharmCAT analysis**</span>, the full PharmCAT report will be also available for download in the summary box.

<div style="text-align: left;"><img src="summary-box-01.png" alt="Summary Box" height="90%" width="90%" max-width="850px"/>

### 4.2 Scatter Plot<a name="scatter-plot"></a>
...
The ranked therapies are plotted as points with different shapes, colours and sizes:

- **Shape** indicates the type of drug-gene association.
- **Colour** indicates the approval status of the drug.
...
### 4.3 Pie Charts<a name="pie-charts"></a>
...
### 4.4 Summary Table<a name="summary-table"></a>
...
**3. Interaction:** Type of drug-gene association. The value of this column in the collapsed table corresponds to the drug-gene pair with the highest DScore and GScore. Options are:
  ...
  - **Indirect:** The drug's target is a gene related to the altered one.
    ...
    - **Genetic dependency<img src="genetic-dependency-interaction.svg" alt="Genetic Depedency" height="20" width="108"/>**
...
**6. Drug response:** <img src="response-sensitivity.svg" alt="Sensitivity" height="35" width="70" style="vertical-align:middle;"/> or <img src="response-resistance.svg" alt="Resistance" height="35" width="75" style="vertical-align:middle;"/> response, based on the gene alteration. Alerts are included in this area:
  ...
  - <img src="response-both.svg" alt="Both" height="35" width="38" style="vertical-align:middle;"/> If there is a predicted <span style="color:#7F0004">**resistance**</span> response to a drug based on a particular gene and another gene indicates <span style="color:#2F7658">**sensitivity**</span>, that drug will be assigned the label <span style="color:#295876">**both**</span> in the **Drug response** column. These indications should be reviewed to decide the drug’s suitability for a particular case, as sometimes the <span style="color:#2F7658">**sensitivity**</span> or <span style="color:#7F0004">**resistance**</span> response is dependent on a particular type of alteration. Likewise, if some gene alteration is associated with <span style="color:#2F7658">**sensitivity**</span> and another alteration in the same gene indicates <span style="color:#7F0004">**resistance**</span>, the drug response for that gene, and thus for the drug, will be <span style="color:#295876">**both**</span>.
...
**7. Family:** Drug family to which the compound belongs. It is based on the [KEGG's Target-based Classification of Drugs](https://www.genome.jp/kegg-bin/get_htext?br08310.keg) and the [Connectivity Map (CMAP)](https://www.broadinstitute.org/connectivity-map-cmap) classification.
...
**8. Source(s):** Source(s) where the drug-gene association comes from. Each source name links to the original resource. Sources are:
  ...
  <img src="pandrugs-sources.png" alt="PanDrugs Sources" height="30%" width="30%" max-width="850px" style="horizontal-align:left;"/>
...
**12. PharmCAT:** This column will appear after a [small variants query](#!/help#vcf-query) or a [multi-omics query](#!/help#multi-omics-query) with <span style="color:#50AC50">**PharmCAT analysis**</span>. The drugs with CPIC recommendations will be labelled with one of these icons:
  ...
  - <img src="strongly-recommended.svg" alt="Strongly Recommended" height="35" width="35" style="vertical-align:middle;"/> **Strongly Recommended:** There is **strong** evidence to **recommend** the administration of this drug according to the patient's germline variants.
  - <img src="moderately-recommended.svg" alt="Moderately Recommended" height="35" width="35" style="vertical-align:middle;"/> **Moderately Recommended:** There is **moderate** evidence to **recommend** the administration of this drug according to the patient's germline variants.
  ...
  - <img src="moderately-not-recommended.svg" alt="Moderately not Recommended" height="35" width="35" style="vertical-align:middle;"/> **Moderately not Recommended:** There is **moderate** evidence to **not recommend** the administration of this drug according to the patient's germline variants.
  - <img src="strongly-not-recommended.svg" alt="Strongly not Recommended" height="35" width="35" style="vertical-align:middle;"/> **Strongly not Recommended:** There is **strong** evidence to **not recommend** the administration of this drug according to the patient's germline variants.
...
**15. Expression:** This column will appear after a [multi-omics query](#!/help#multi-omics-query) with expression data. A drug will be labelled with this icon <img src="expression.svg" alt="Expression" height="35" width="35" style="vertical-align:middle;"/> when any of its associated genes are categorized as
...
Each row has **ADDITIONAL INFORMATION** that can be expanded by clicking on the <img src="plus.png" alt="Expand Icon" height="18" width="20" style="vertical-align:middle;"/> button. Columns 6, 8 to 10 and 13 to 15 are further detailed for each gene involved in the drug assignment. Moreover, a new section for each gene is shown on the left. This section contains:
  ...
  - **After a [multi-omics query](#!/help#multi-omics-query):** A sentence explaining the coherence between the alterations found in the different input files is provided.
...
### 4.5 PanDrugs Report<a name="report"></a>

After a [small variants query](#!/help#vcf-query), a downloadable report with PanDrugs' top results will appear in the [summary box](#!/help#summary-box). This report is aimed to support clinical decision-making and comprises two sections:

- **Summary section:** A brief overview of PanDrugs results, including information detailed in the summary box and charts.
...
Load <span style="color:#50AC50">**Example 3**</span> from the [genes tab.](#!/query?tab=genes)
...
Moreover, alpelisib and temsirolimus are both targeted therapies approved for cancer and belong to the PI3K inhibitor and mTOR inhibitor families respectively.
...
1. A pathway member association with the direct target *MTOR*, which is downstream *AKT1*, *AKT2*, *PDPK1*, *PIK3CA*, *PIK3R1*, *PIK3R2*, *RHEB*, *TSC1* and *TSC2*. You can click on <span style="color:#50AC50">**See pathways**</span> to retrieve more information regarding the functional pathways in which they are involved.
2. A direct target inhibition: *MTOR*.
3. Three response biomarkers: *PIK3CA*, *PTEN* and *AKT1*.
...
- **Pathway members:** Such as the direct target *CCND1*, which is downstream of *ARAF*, *BRAF*, *KRAS*, *MAP2K1*, *RAF1* and *TP53*. Note that *CCND1* GScore as a pathway member (DScore = 1) is different from its score as a direct target (DScore = 0.7257) since the first one is computed as the maximum GScore of all upstream genes.
...
This example corresponds to patient [TCGA-91-6847](https://www.cbioportal.org/patient?studyId=luad_tcga_pan_can_atlas_2018&caseId=TCGA-91-6847), who harbours an amplification in *EGFR* that leads to an increased expression of this gene. The example .rnk file contains the top 500 highly expressed genes. The ranking metric in this case is the statistic of the differential expression test.
...
<div style="text-align: left;"><img src="gene-ranking-query-output-01.png" alt="Scatter Plot Gene Ranking Query" height="60%" width="60%" max-width="850px"/>
...
This gene has no direct association with any drug in PanDrugsdb. Nevertheless, *CALML5* is involved in a lot of different pathways and some of its downstream genes are direct targets of PanDrugs therapies. Thus, all these BTCs are obtained via pathway member <img src="pathway-member-interaction.svg" alt="Pathway Member" height="20" width="108" style="vertical-align:middle;"/> associations.
...
We know that patients with *BRCA2* deletion are susceptible to PARP inhibitors. If we scroll down the table, we'll discover some drugs from this family that have a biomarker <img src="biomarker-interaction.svg" alt="Biomarker" height="20" width="68" style="vertical-align:middle;"/> association with *BRCA2* and a DScore > 0.7. However, *BRCA2* GScore is below the threshold for defining the BTCs (GScore < 0.6).
...
### 6.5 Small Variants Analysis with Somatic Alterations<a name="examples-vcf-query-somatic-variants"></a>
...
If you come back to the small variants tab, you will notice a progress bar associated with your job. As the computation progresses, the completion of the different steps will be indicated here. In this example, PanDrugs will annotate the somatic variants and compute GScores from VScores. For more details, please refer to the [Small Variants Query](#!/help#vcf-query) section.
...
On the results page, you will see a summary box with the execution details, some plots, a summary table with a drug ranking and a downloadable report with top results (see [Gene Names, Gene Ranking, CNVs, Small Variants and Multi-omics Query Output](#!/help#genes-gene-rank-cnvs-vcf-and-multi-omics-query-output) section for further details).
...
PanDrugs returns several Best Therapeutic Candidates <img src="btc.png" alt="Best Therapeutic Candidate" height="20" width="20" style="vertical-align:middle;"/> associated with *PIK3CA*. Among these BTCs there are PI3K inhibitors such as alpelisib and copanlisib.
...
PanDrugs report with the BTCs is available at the top of the page.

### 6.6 Small Variants Analysis with Somatic and Germline Alterations<a name="examples-vcf-query-somatic-germline-variants"></a>

#### 6.6.1 Small Variants Analysis without PharmCAT<a name="examples-vcf-query-no-pharmcat"></a>

[Load example for a colon adenocarcinoma patient from the TCGA with synthetic germline variants.](#!/query?tab=vcfrank)

This example corresponds to patient [TCGA-AA-3696](https://www.cbioportal.org/patient?studyId=coadread_tcga&caseId=TCGA-AA-3696), who harbours two mutations in *KRAS* and *PIK3CA*. The VCF contains the patient's somatic variants as well as made-up germline variants to simulate how PharmCAT analysis would enrich the final drug ranking in a real-case scenario.
...
For this example, PanDrugs finds several Best Therapeutic Candidates <img src="btc.png" alt="Best Therapeutic Candidate" height="20" width="20" style="vertical-align:middle;"/>. If you are interested in a drug repurposing approach using hormone therapy, you may want to consider tamoxifen (DScore = 0.9360; GScore = 0.6745), which is approved for breast cancer.
...
#### 6.6.2 Small Variants Analysis with PharmCAT<a name="examples-vcf-query-pharmcat"></a>
...
PanDrugs will annotate the somatic variants and compute GScores from VScores. In addition, PanDrugs will query PharmCAT with the germline variants to retrieve the CPIC recommendations for the drugs shown in the final ranking. For more details, please refer to the [Small Variants Query](#!/help#vcf-query) section.
...
The results page will look exactly the same as the output of the [Small Variants Analysis without PharmCAT](#!/help#examples-vcf-query-no-pharmcat). However, the summary table will show an additional column named **PharmCAT**.

Please, take a look at the tamoxifen entry. There is a PharmCAT label that indicates that this patient has germline variants that are associated with Adverse Drug Reactions (ADRs) to this drug. If you click on the icon, you will be redirected to tamoxifen's section in PharmCAT's report, which further explains the administration guideline.
...
Moreover, PharmCAT and PanDrugs reports can be downloaded from the summary box at the top of the page.
...
### 6.7. Multi-omics Analysis<a name="examples-multi-omics-query"></a>
...
We have already analyzed data from this patient in the [CNVs Analysis](#!/help#examples-cnvs-query) and the [Small Variants Analysis](#!/help#examples-vcf-query) sections. In the following example, we will show how PanDrugs' results can be enriched using different omics data from the same patient.

#### 6.7.1 Multi-omics Analysis without VCF<a name="examples-multi-omics-query-wo-vcf"></a>
...
These results can be interpreted as the ones obtained after a [gene names analysis](#!/help#examples-genes-query). PanDrugs finds several Best Therapeutic Candidates <img src="btc.png" alt="Best Therapeutic Candidate" height="20" width="20" style="vertical-align:middle;"/> based on CNV and expression evidence. 
...
If we expand the everolimus row by clicking on the <img src="plus.png" alt="Expand Icon" height="18" width="20" style="vertical-align:middle;"/>, we will get further information about the type of gene alterations that are contributing to PanDrugs ranking. In the expanded table, we observe that both *RB1* and *BRCA2*, biomarkers of everolimus, are deleted and this deletion leads to gene underexpression.
...
#### 6.7.2 Multi-omics Analysis with VCF<a name="examples-multi-omics-query-w-vcf"></a>
...
For the sake of this example, we will run this analysis from scratch. First, go to the [small variants query tab](#!/query?tab=vcfrank) and load the VCF as indicated in the section [Small Variants Analysis without PharmCAT](#!/help#examples-vcf-query-no-pharmcat). Please, **do not** click on <span style="color:#50AC50">**PharmCAT analysis**</span>.

<div style="text-align: left;"><img src="vcf-query-example-input-01.png" alt="Multi-omics Query with VCF Input 1" height="40%" width="40%" max-width="850px"/>

Once the progress bar is complete, return to the [multi-omics query tab](#!/query?tab=multiomics).
...
PanDrugs finds several Best Therapeutic Candidates <img src="btc.png" alt="Best Therapeutic Candidate" height="20" width="20" style="vertical-align:middle;"/> based on small variant, CNV and expression evidence. Afatinib is the drug with the highest combined DScore and GScore (DScore = 0.9280; GScore = 0.7995) according to the 3 evidence sources.
...
If we expand the afatinib row by clicking on the <img src="plus.png" alt="Expand Icon" height="18" width="20" style="vertical-align:middle;"/>, we will get further information about the type of gene alterations that are contributing to PanDrugs ranking:

  - A mutation in *PIK3CA* that leads to high overexpression of this gene.
  - An amplification in *JPH1* that also leads to high overexpression of this gene.
